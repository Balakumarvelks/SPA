...
const Signin = ({ makeItSpin }) => {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const userRef = ref(db, 'customers/' + response.user.uid);
      ...
      if (snapshot.exists()) {
        ...
        Cookies.set('malar_client_uid', response.user.uid);
        navigate('/');
      } else {
      ...
      <div className='container lg:max-w-sm max-w-md '>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='flex flex-col gap-4'>
            ...
            <p className='text-center font-bold cursor-pointer'>Don't have an account ? <span onClick={() => navigate('/signup')}>
              Sign Up
            </span> </p>
          </div>
        </form>
